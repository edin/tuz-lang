...
#define TEST_ASSERT_TRUE(expr) \
    do { \
        bool result = (expr); \
        tuz::test::TestRunner::instance().add_result(result, \
            std::string("ASSERT_TRUE failed: ") + #expr, __FILE__, __LINE__); \
    } while(0)
...
#define TEST_ASSERT_FALSE(expr) \
    do { \
        bool result = !(expr); \
        tuz::test::TestRunner::instance().add_result(result, \
            std::string("ASSERT_FALSE failed: ") + #expr, __FILE__, __LINE__); \
    } while(0)
...
#define TEST_ASSERT_THROW(expr, exception_type) \
    do { \
        ...
        tuz::test::TestRunner::instance().add_result(caught, \
            std::string("ASSERT_THROW failed: expected ") + #exception_type, __FILE__, __LINE__); \
    } while(0)
